...
extern int currentIndex = 0;
extern int currentIndexTemp = 0;
extern int flag = 0;
...
Token *back_token() 
{
	...
	return &currentNode->tokensArray[--currentIndexTemp + currentIndex];
}
...
Token *next_token()
{
	...
	{
		if (currentIndexTemp == 0 && currentNode->tokensArray[currentIndex].kind == TOKEN_EOF)
		{
				return &currentNode->tokensArray[currentIndex];
		}
		else if(currentIndexTemp == 0)
		{
			yylex();
			return &currentNode->tokensArray[currentIndex];
		}
		...
		{
			int temp = ++currentIndexTemp + currentIndex;
			return &currentNode->tokensArray[temp];
		}
	}
}
